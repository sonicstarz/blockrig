...
Planned 2026-08-02, before any code. Supersedes the stage/row chain model
(docs/12 §chain engine) and the lane UI (docs/13): free-form routing — drag
blocks on a snapping grid, wire them visually, split and merge signal paths by
gesture instead of the per-stage A/B menu.
