...
- **`<State>`**: exactly what the hosted instance's `getStateInformation` produced, stored opaquely and handed straight back to `setStateInformation` on the same plug-in. **Never synthesize, edit, or migrate a child chunk** — measured in P0: JUCE's plug-in wrapper nests the processor chunk inside its own container, so a foreign or hand-built chunk is *silently* ignored (148 KB in, defaults out, no error). Because failure is silent, the loader must verify a restore actually took (compare one known parameter, or a checksum of the instance's post-restore state) and mark the block as "state not restored" in the UI rather than pretending success. Note also that hosted parameter changes only reach a plug-in while audio flows, so state-level restore is always preferred over replaying parameter values. No size cap enforced; warn in UI above ~20 MB total rig size (folklore host-chunk risk zone).
